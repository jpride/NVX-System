...
namespace TSISignageApp.UI
{
	public static class SmartGraphicsHelper
	{
		...
		public static void SetSmartObjectDigitalJoinByName ( SmartObject so, string name, bool state )
		{
			try
			{
		...
		public static void SetSmartObjectSelected ( SmartObject so, int index, bool state )
		{
			...
			if (so.StringInput.Contains ( name ))
				SetSmartObjectDigitalJoinByName ( so, name, state );
			else
			{
		...
		//SmartObject by Join for SRLs or Lists that dont have their sgd definition imported for whatever reason
		public static void SetSmartObjectDigitalJoinByJoin ( SmartObject so, int index, bool state )
		{
			try
			{
				so.BooleanInput[ (ushort)index ].BoolValue = state;
		...
		public static void PulseSmartObjectDigitalJoinByJoin ( SmartObject so, int index, int msPulseTime)
		{
			try
			{
				so.BooleanInput[ (ushort)index ].Pulse ( msPulseTime );
			}
			catch (Exception e)
			{
				CrestronConsole.PrintLine ( "SetSmartObjectDigitalJoin exception: {0}", e.Message );
		...
		public static void SetSmartObjectTextByJoin ( SmartObject so, ushort join, string state )
		{
			try
			{
				so.StringInput[ join ].StringValue = state;
			}
			catch (Exception e)
			{
				CrestronConsole.PrintLine ( "SetSmartObjectText exception: {0}", e.Message );
		...
		{
			CrestronConsole.PrintLine ( "SmartObject Object ID {0}, on {1}", so.ID, so.Device.Name.ToString ( ) );
